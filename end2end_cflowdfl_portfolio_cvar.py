...
def create_datasets(x, c, batch_size, weights, caps, contextual, risk=False):
    x_train, x_test, c_train, c_test = train_test_split(x, c, test_size=int(x.shape[0]*0.2), random_state=246)
    
    optmodel = ExpectedPortfolioModel(c_test.shape[1], weights, risk=risk)
...
def evaluate_model(gen_model, contextual, optmodel, loader_test, batch_size, device):
    ...
    alphas = [i for i in np.arange(0.01, 1, 0.01)]
    cvars = {
        f"alpha_{alpha}": {
            "objective": [],
            "regret": []
    ...
    for data in tqdm(loader_test, desc="Evaluating"):
        ...
        for alpha in alphas:
            obj = optmodel.obj_eval(x, cp_samples, c_trues, alpha=alpha)
            cvars[f"alpha_{alpha}"]["objective"].append(obj)  
            obj = optmodel.regret_loss_batch(x, cp_samples, c_trues, alpha=alpha)
            cvars[f"alpha_{alpha}"]["regret"].append(obj / (z.abs().sum().item() + 1e-7))
        ...
        # CVaR 0.75 objective
        obj = optmodel.obj_eval(x, cp_samples, c_trues, alpha=0.75)
        cvar_075_objectives.append(obj)# / (z.abs().sum().item() + 1e-7))
        
        # CVaR 0.75 regret
        obj = optmodel.regret_loss_batch(x, cp_samples, c_trues, alpha=0.75)
        cvar_075_regrets.append(obj / (z.abs().sum().item() + 1e-7))
    
        # CVaR 0.25 objective
        obj = optmodel.obj_eval(x, cp_samples, c_trues, alpha=0.25)
        cvar_025_objectives.append(obj)# / (z.abs().sum().item() + 1e-7))
        
        # CVaR 0.25 regret
        obj = optmodel.regret_loss_batch(x, cp_samples, c_trues, alpha=0.25)
        cvar_025_regrets.append(obj / (z.abs().sum().item() + 1e-7))

        # CVaR 0.2 objective
        obj = optmodel.obj_eval(x, cp_samples, c_trues, alpha=0.2)
        cvar_02_objectives.append(obj)# / (z.abs().sum().item() + 1e-7))
        
        # CVaR 0.2 regret
        obj = optmodel.regret_loss_batch(x, cp_samples, c_trues, alpha=0.2)
        cvar_02_regrets.append(obj / (z.abs().sum().item() + 1e-7))

        # CVaR 0.3 objective
        obj = optmodel.obj_eval(x, cp_samples, c_trues, alpha=0.3)
        cvar_03_objectives.append(obj)# / (z.abs().sum().item() + 1e-7))
        
        # CVaR 0.3 regret
        obj = optmodel.regret_loss_batch(x, cp_samples, c_trues, alpha=0.3)
        cvar_03_regrets.append(obj / (z.abs().sum().item() + 1e-7))

        # CVaR 0.4 objective
        obj = optmodel.obj_eval(x, cp_samples, c_trues, alpha=0.4)
        cvar_04_objectives.append(obj)# / (z.abs().sum().item() + 1e-7))
        
        # CVaR 0.4 regret
        obj = optmodel.regret_loss_batch(x, cp_samples, c_trues, alpha=0.4)
        cvar_04_regrets.append(obj / (z.abs().sum().item() + 1e-7))
    ...
    print(f"CVaR 0.25 Objective: {np.mean(cvar_025_objectives)}")
    print(f"CVaR 0.25 Regret: {np.mean(cvar_025_regrets)}")
    print(f"CVaR 0.75 Objective: {np.mean(cvar_075_objectives)}")
    print(f"CVaR 0.75 Regret: {np.mean(cvar_075_regrets)}")

    cvars_mean = {
        f"alpha_{alpha}": {
            "objective": float(np.mean(cvars[f"alpha_{alpha}"]["objective"])),
            "regret": float(np.mean(cvars[f"alpha_{alpha}"]["regret"]))
        } for alpha in alphas
    }
    return np.mean(average_objectives), np.mean(average_regrets), cvars_mean, np.mean(cvar_objectives), np.mean(cvar_regrets), np.mean(cvar_01_objectives), np.mean(cvar_01_regrets)
...
def run_experiment(m, n, p, deg, dim, noise_width, caps, batch_size, num_epochs, device, beta, alpha, rank=None, risk=False):
    weights, x, c, contextual = generate_data(m, n, p, deg, dim, noise_width, caps, rank=rank)
    loader_train, loader_test, optmodel = create_datasets(x, c, batch_size, weights, caps, contextual, risk=risk)
    ...
    average_objective, average_regret, cvars, cvar_objective, cvar_regret, cvar_01_objective, cvar_01_regret = evaluate_model(trained_model, contextual, optmodel, loader_test, batch_size, device)
    
    return trained_model, average_objective, average_regret, cvars, cvar_objective, cvar_regret, cvar_01_objective, cvar_01_regret, dfl_losses, nll_losses, (weights, x, c)
        ...
        for m in m_values:
            ...
            for alpha in alphas:
                for exp_idx in range(num_experiments):
                    ...
                    model, avg_objective, avg_regret, cvars, cvar_objective, cvar_regret,\
                          cvar_01_objective, cvar_01_regret, dfl_losses, nll_losses, (weights, x, c) = run_experiment(
                        m, n, p, deg, dim, noise_width, caps, batch_size, num_epochs, device, beta, alpha, rank=args.rank, risk=args.risk
                    )
                    ...
                    cvars_results.append(cvars)
                    
                    # save the data
                    os.makedirs(f"eval/portfolio_cvar/beta_{beta}_alpha_{alpha}_risk_{args.risk}/data", exist_ok=True)
                    np.save(f"eval/portfolio_cvar/beta_{beta}_alpha_{alpha}_risk_{args.risk}/data/end2end_cflowdfl_data_exp_{exp_idx}_m{m}_n{n}_noise_{noise_width}_deg{deg}_weights.npy", weights)
                    np.save(f"eval/portfolio_cvar/beta_{beta}_alpha_{alpha}_risk_{args.risk}/data/end2end_cflowdfl_data_exp_{exp_idx}_m{m}_n{n}_noise_{noise_width}_deg{deg}_x.npy", x)
                    np.save(f"eval/portfolio_cvar/beta_{beta}_alpha_{alpha}_risk_{args.risk}/data/end2end_cflowdfl_data_exp_{exp_idx}_m{m}_n{n}_noise_{noise_width}_deg{deg}_c.npy", c)
                    
                    # Save loss curves as numpy arrays
                    os.makedirs(f"eval/portfolio_cvar/beta_{beta}_alpha_{alpha}_risk_{args.risk}/losses", exist_ok=True)
                    np.save(f"eval/portfolio_cvar/beta_{beta}_alpha_{alpha}_risk_{args.risk}/losses/end2end_cflowdfl_dfl_losses_exp_{exp_idx}_m{m}_n{n}_noise_{noise_width}.npy", np.array(dfl_losses))
                    np.save(f"eval/portfolio_cvar/beta_{beta}_alpha_{alpha}_risk_{args.risk}/losses/end2end_cflowdfl_nll_losses_exp_{exp_idx}_m{m}_n{n}_noise_{noise_width}.npy", np.array(nll_losses))
                    ...
                    plt.legend()
                    plt.savefig(f"eval/portfolio_cvar/beta_{beta}_alpha_{alpha}_risk_{args.risk}/losses/end2end_cflowdfl_dfl_loss_exp_{exp_idx}_m{m}_n{n}_noise_{noise_width}.png")
                    plt.close()
                    ...
                    plt.legend()
                    plt.savefig(f"eval/portfolio_cvar/beta_{beta}_alpha_{alpha}_risk_{args.risk}/losses/end2end_cflowdfl_nll_loss_exp_{exp_idx}_m{m}_n{n}_noise_{noise_width}.png")
                    plt.close()
                    
                    # Save model weights
                    os.makedirs(f"eval/portfolio_cvar/beta_{beta}_alpha_{alpha}_risk_{args.risk}", exist_ok=True)
                    torch.save(model.state_dict(), f"eval/portfolio_cvar/beta_{beta}_alpha_{alpha}_risk_{args.risk}/end2end_cflowdfl_model_exp_{exp_idx}_m{m}_n{n}_noise_{noise_width}.pth")
                    
                    # Save results
                    with open(f"eval/portfolio_cvar/beta_{beta}_alpha_{alpha}_risk_{args.risk}/end2end_cflowdfl_results_exp_{exp_idx}_m{m}_n{n}_noise_{noise_width}.json", "w") as f:
                        json.dump(result, f, indent=2)

                    # Save cvars
                    with open(f"eval/portfolio_cvar/beta_{beta}_alpha_{alpha}_risk_{args.risk}/end2end_cflowdfl_cvars_exp_{exp_idx}_m{m}_n{n}_noise_{noise_width}.json", "w") as f:
                        json.dump(cvars, f, indent=2, default=float)  # Add default=float to handle numpy types

        # Save all results in a single file
        with open(f"eval/portfolio_cvar/all_results_end2end_deg{deg}_betas{beta}_alpha_{alpha}_m{m}_noise_{noise_width}_n{n}_risk_{args.risk}.json", "w") as f:
            json.dump(results, f, indent=2)

        # Save all cvars in a single file
        with open(f"eval/portfolio_cvar/all_cvars_end2end_deg{deg}_betas{beta}_alpha_{alpha}_m{m}_noise_{noise_width}_n{n}_risk_{args.risk}.json", "w") as f:
            json.dump(cvars_results, f, indent=2, default=float)  # Add default=float to handle numpy types


    # Save all results in a single file
    with open(f"eval/portfolio_cvar/all_results_end2end_betas{beta}_alpha_{alpha}_m{m}_noise_{noise_width}_n{n}_risk_{args.risk}.json", "w") as f:
        json.dump(results, f, indent=2)
