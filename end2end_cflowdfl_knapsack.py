...
def create_datasets(x, c, batch_size, weights, caps, contextual):
    ...
    optmodel = ExpectedKnapsackModel(weights, caps)
...
def run_experiment(m, n, p, deg, dim, noise_width, caps, batch_size, num_epochs, device, beta, alpha, rank=None):
    ...
    optimizer = torch.optim.Adam(gen_model.parameters(), lr=0.01)
...
def main():
    ...
    parser.add_argument("--betas", type=float, default=100)
        ...
        for m in m_values:
            ...
            for alpha in alphas:
                for exp_idx in range(num_experiments):
                    ...
                    os.makedirs(f"eval/knapsack/beta_{beta}_alpha_{alpha}/losses", exist_ok=True)
                    np.save(f"eval/knapsack/beta_{beta}_alpha_{alpha}/losses/end2end_cflowdfl_dfl_losses_exp_{exp_idx}_m{m}_n{n}_noise_{noise_width}.npy", np.array(dfl_losses))
                    np.save(f"eval/knapsack/beta_{beta}_alpha_{alpha}/losses/end2end_cflowdfl_nll_losses_exp_{exp_idx}_m{m}_n{n}_noise_{noise_width}.npy", np.array(nll_losses))
                    ...
                    plt.legend()
                    plt.savefig(f"eval/knapsack/beta_{beta}_alpha_{alpha}/losses/end2end_cflowdfl_dfl_loss_exp_{exp_idx}_m{m}_n{n}_noise_{noise_width}.png")
                    plt.close()
                    ...
                    plt.legend()
                    plt.savefig(f"eval/knapsack/beta_{beta}_alpha_{alpha}/losses/end2end_cflowdfl_nll_loss_exp_{exp_idx}_m{m}_n{n}_noise_{noise_width}.png")
                    plt.close()
                    
                    # Save model weights
                    os.makedirs(f"eval/knapsack/beta_{beta}_alpha_{alpha}", exist_ok=True)
                    torch.save(model.state_dict(), f"eval/knapsack/beta_{beta}_alpha_{alpha}/end2end_cflowdfl_model_exp_{exp_idx}_m{m}_n{n}_noise_{noise_width}.pth")
                    
                    # Save results
                    with open(f"eval/knapsack/beta_{beta}_alpha_{alpha}/end2end_cflowdfl_results_exp_{exp_idx}_m{m}_n{n}_noise_{noise_width}.json", "w") as f:
                        json.dump(result, f, indent=2)
            
    # Save all results in a single file
    with open(f"eval/knapsack/all_results_end2end_betas{beta}_alpha_{alpha}_m{m}_noise_{noise_width}.json", "w") as f:
